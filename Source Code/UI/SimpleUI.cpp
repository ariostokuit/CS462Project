  ...
  void SimpleUI::launch()
  {
    ...
    do
    {
      ...
      ******************************************************************************************************************************/
      ...
      // }
      if( selectedCommand == "Purchase Products" )
      {
        std::vector<std::string> checkoutParameters( 0 );

        std::cout << " Initiating checkout...\n";
        auto receipt = sessionControl->executeCommand("Initate Checkout", checkoutParameters);
        if( receipt.has_value() ) _logger << "Received reply: \"" + std::any_cast<const std::string &>( receipt ) + '"';

        std::vector<std::string> orderParameters( 3 );
        std::cout << " How do you want your order to be delivered?\n";
        std::cout << " 1. Pick up at store\n";
        std::cout << " 2. Delivered to specific address\n";
        std::cin >> std::ws;  std::getline( std::cin, orderParameters[0] );
        std::cout << " Address to be delivered to (Put N/A if you are picking up at store): ";  std::cin >> std::ws;  std::getline( std::cin, orderParameters[1] );
        std::cout << " Any miscellaneous instruction for delivery: ";  std::cin >> std::ws;  std::getline( std::cin, orderParameters[2] );
        // auto order = sessionControl->executeCommand("Order Instructions", orderParameters);
        // if( receipt.has_value() ) _logger << "Received reply: \"" + std::any_cast<const std::string &>( order ) + '"';

        // auto results = sessionControl->executeCommand( selectedCommand, order );
        // if( results.has_value() ) _logger << "Received reply: \"" + std::any_cast<const std::string &>( results ) + '"';
      }
